...
import (
	...
	"prestoBackend/src/internal/medidor/service"
	"time"
...
func (controller *MedidorController) CrearMedidor(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		common.ResponseJSON(w, http.StatusUnprocessableEntity, map[string]string{"mensaje": err.Error()})
		return
	}
...
func (controller *MedidorController) EliminarMedidor(w http.ResponseWriter, r *http.Request) {
	...
	ID, err := common.ValidadIdMongo(medidor)
...
func (controller *MedidorController) ActualizarMedidor(w http.ResponseWriter, r *http.Request) {
	...
	ID, err := common.ValidadIdMongo(id)
	if err != nil {
		common.ResponseJSON(w, http.StatusBadRequest, map[string]string{"mensaje": err.Error()})
		return
	}
