...
type MedidorRepository interface {
	...
	ListarMedidorCliente(filter *dto.BuscadorMedidorClienteDto, ctx context.Context) (*appDto.ResultadoPaginado, error)
...
func (r *medidorRepository) ListarMedidorCliente(filter *dto.BuscadorMedidorClienteDto, ctx context.Context) (*appDto.ResultadoPaginado, error) {
	...
	if filter.NumeroMedidor != "" {
		pipeline = append(pipeline, aggregation.RegexMatch("numeroMedidor", filter.NumeroMedidor))
	}
	...
	if filter.Direccion != "" {
		pipeline = append(pipeline, aggregation.RegexMatch("direccion", filter.Direccion))
	}
	pipeline = append(pipeline,
		aggregation.Lookup("Cliente", "cliente", "_id", "cliente"),
		aggregation.Unwind("$cliente", false),
	)

	if filter.Codigo != "" {
		pipeline = append(pipeline, aggregation.RegexMatch("cliente.codigo", filter.Codigo))
	}
	if filter.Nombre != "" {
		pipeline = append(pipeline, aggregation.RegexMatch("cliente.nombre", filter.Nombre))
	}

	if filter.ApellidoPaterno != "" {
		pipeline = append(pipeline, aggregation.RegexMatch("cliente.apellidoPaterno", filter.ApellidoPaterno))
	}
	if filter.ApellidoMaterno != "" {
		pipeline = append(pipeline, aggregation.RegexMatch("cliente.apellidoMaterno", filter.ApellidoMaterno))
	}

	pipeline = append(pipeline,
		aggregation.Lookup("Tarifa", "tarifa", "_id", "tarifa"),
		aggregation.Unwind("$tarifa", false),
	)
	if filter.Tarifa != "" {
		ID, err := common.ValidadIdMongo(filter.Tarifa)
		if err != nil {
			return nil, err
		}
		fmt.Println(ID)
		pipeline = append(pipeline, aggregation.Match("tarifa._id", ID))
	}
	...
	var pipeline mongo.Pipeline = mongo.Pipeline{
		...
		bson.D{
			{Key: "$project", Value: bson.D{
				...
				{Key: "apellidoMaterno", Value: "$cliente.apellidoMaterno"},
				{Key: "codigo", Value: "$cliente.codigo"},
				{Key: "tarifa", Value: aggregation.ArrayElemAt("$tarifa.nombre", 0)},
			}},
		},
	}
	cursor, err := r.collection.Aggregate(ctx, pipeline)
...
func (r *medidorRepository) ObtenerMedidorConClientePorId(medidorId *bson.ObjectID, ctx context.Context) (*[]bson.M, error) {
	var pipeline mongo.Pipeline = mongo.Pipeline{
		...
		bson.D{
			{Key: "$project", Value: bson.D{
				{Key: "_id", Value: 1},
				{Key: "idCliente", Value: aggregation.ArrayElemAt("$cliente._id", 0)},
				{Key: "nombre", Value: aggregation.ArrayElemAt("$cliente.nombre", 0)},
				{Key: "apellidoPaterno", Value: aggregation.ArrayElemAt("$cliente.apellidoPaterno", 0)},
				{Key: "apellidoMaterno", Value: aggregation.ArrayElemAt("$cliente.apellidoMaterno", 0)},
